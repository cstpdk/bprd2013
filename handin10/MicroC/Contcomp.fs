...
let rec addIFZRO lab (C : instr list) = 
    match C with
    | IFZERO lab1 :: GOTO lab2 :: Label lab3 :: crest when compare lab3 lab1 = 0 -> IFNZRO lab2 :: Label lab3 :: crest
    | _ -> IFZERO lab :: C
let addGOTO lab C =
    addJump (GOTO lab) C
...
let rec addCST i C =
    ...
    | (i1, CSTI i2 :: EQ   :: NOT :: C1) -> if (i1 = i2) then (CSTI 0 :: C1) else (CSTI 1 :: C1)
    | (i1, CSTI i2 :: SWAP :: LT :: C1) -> if (i1 > i2) then (CSTI 1 :: C1) else (CSTI 0 :: C1) 
    | (i1, CSTI i2 :: SWAP :: LT :: NOT :: C1) -> if (i1 <= i2) then (CSTI 1 :: C1) else (CSTI 0 :: C1)     
    | _                     -> CSTI i :: C
...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) (C : instr list) : instr list = 
    ...
    | If(e, stmt1, stmt2) -> 
      ...
      cExpr e varEnv funEnv (addIFZRO labelse (cStmt stmt1 varEnv funEnv (addJump jumpend C2)))
    | While(e, body) ->
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) (C : instr list) : instr list =
    ...
    | Cond(e1,e2,e3) -> 
        let (L2,C2) = addLabel C
        let (L1,C1) = addLabel (cExpr e3 varEnv funEnv C2)
        cExpr e1 varEnv funEnv 
            (IFZERO L1 :: 
            (cExpr e2 varEnv funEnv (GOTO L2 :: C1))
            )
    | Prim1(ope, e1) ->
      cExpr e1 varEnv funEnv
          (match ope with
    ...
    | Andalso(e1, e2) ->
      match C with
      | IFZERO lab :: _ ->
         cExpr e1 varEnv funEnv (addIFZRO lab (cExpr e2 varEnv funEnv C))
      | IFNZRO labthen :: C1 -> 
